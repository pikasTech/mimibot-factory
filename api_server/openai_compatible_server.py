...
class OpenAICompatibleServer:
    ...
    def start(self, daemon=True):
        """启动API服务"""
        print(f"Starting OpenAI compatible API service on {self.host}:{self.port}{self.base_path}")
        ...
        def chat_completions():
            try:
                ...
                # 准备提示
                prompt = self._prepare_chat_prompt(messages)
                ...
                if stream:
                    ...
                    if self.simulation_mode:
                        def generate():
                            ...
                            # 生成模拟的流式文本
                            chunks = self._stream_simulation_text(prompt, max_tokens)
                        ...
                        def generate_stream():
                            ...
                            while time.time() - start_time < timeout:
                                if request_id in self.result_dict:
                                    result = self.result_dict.get(request_id, {})
                                    
                                    # 检查是否有错误
                                    if "error" in result:
                                        # 发送错误信息
                                        error_msg = json.dumps({"error": {"message": result["error"], "type": "inference_error"}})
                                    ...
                                    for i in range(last_chunk_index + 1, len(chunks)):
                                        chunk = chunks[i]
                                        is_last = (result.get("stream_complete", False) and i == len(chunks) - 1)
                            ...
                            # 如果超时，发送超时消息并结束流
                            error_msg = json.dumps({"error": {"message": "Request timeout", "type": "timeout_error"}})
                            yield f"data: {error_msg}\n\n"
                            yield "data: [DONE]\n\n"
                            # 清理资源
                            if request_id in self.result_dict:
                ...
                for output in outputs:
                    if output.outputs:
                        ...
                        if len(new_text) > len(prev_text):
                            ...
                            self.result_dict[oldest_request_id]["chunks"].append(new_chunk)
...
class OpenAICompatibleCallback(TrainerCallback):
    ...
    def __init__(self, port=8000, simulation_mode=False):
        """
        参数:
            port: API服务器端口
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        if not self.trainer:
            print("Warning: Trainer not found in kwargs")
            return control
        
        if self.trainer.accelerator.is_main_process:
            self.server = OpenAICompatibleServer(port=self.port, simulation_mode=self.simulation_mode)
